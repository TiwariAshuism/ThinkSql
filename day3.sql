-- Note:
-- please do not
-- use any functions which are not taught in the class. you need to solve the questions only
-- with the concepts that have been discussed so far.
...
-- 3- write a query to
-- print dep
-- name and average salary of employees in that dep .
...
-- 4- write a query to
-- print dep
-- names where none of the emplyees have same salary.
...
-- 5- write a query to
-- print sub
-- categories where we have all 3 kinds of returns
...
select top 3
    sub_category, sum(o.sales) as return_sales
from orders o
    inner join returns r on o.order_id=r.order_id
where o.region='East'
...
-- 8- write a query to
-- print dep
-- name for which there is no employee
...
-- 9- write a query to
-- print employees
-- name for which dep id is not avaiable in dept table
